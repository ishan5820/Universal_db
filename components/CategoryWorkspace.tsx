    ...
    <main className="mx-auto w-full max-w-[1700px] px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
      <header className="mb-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex items-center gap-4"><span style={categoryAccentStyle(colors, category)} className="h-12 w-2 rounded-full" aria-hidden="true" /><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Category workspace</p><h1 className="mt-1 text-3xl font-bold tracking-[-0.035em] text-slate-950">{style.label}</h1></div></div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={() => { setSeriesToEdit([]); setRecurringOpen(true); }} style={categoryAccentStyle(colors, category)} className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold text-white shadow-sm"><Plus className="h-4 w-4" />Add recurring</button>
          </div>
        </div>
      </header>
