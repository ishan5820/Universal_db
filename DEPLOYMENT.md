...
4. Apply every pending file in `supabase/migrations/` to the linked Universal_db Supabase project before deploying the matching application code. The Stage 6 application expects `calendar_classes`, `calendar_items.class_id`, and `user_settings.unassigned_color` to exist.
5. Run the database RLS tests and advisors, then run `pnpm verify-release` and the full verification commands in the README.

## Deployment window

1. Pause the production site only when the final branch is ready to merge.
2. Commit and push the reviewed branch only after the database migration and checks pass so Vercel builds the production deployment.
...
2. Confirm calendar and class row counts rise only after those users complete their first backup.
