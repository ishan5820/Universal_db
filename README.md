...
`verify-release` checks the two required browser-safe environment variables, reports whether optional server maintenance credentials are present, confirms Google authentication is enabled, and verifies anonymous calendar and class reads are denied. It never prints key values. Follow [DEPLOYMENT.md](./DEPLOYMENT.md) when moving existing browser-only users to cloud backup.
